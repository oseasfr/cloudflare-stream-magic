...
export const onRequestPut: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    if (!minioRes.ok) {
      const text = await minioRes.text();
      return new Response(
        JSON.stringify({ error: "Erro ao enviar para o MinIO.", detail: text }),
        { status: minioRes.status, headers: { "Content-Type": "application/json", ...CORS_HEADERS } }
      );
    }
